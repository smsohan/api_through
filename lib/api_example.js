(function() {
  ...
  ApiExample.prototype.stripResponseBody = function() {
    ...
    return this.strippedResponseBody = strippedObject.strip(JSON.parse(this.responseBody));
  };
  ...
  ApiExample.prototype.setFullUrl = function(isSSL, hostPort) {
    ...
    this.fullURL = "" + scheme + "://" + host + port + this.url;
    return console.log("@fullURL = " + this.fullURL);
  };

  ApiExample.prototype.computeDigest = function() {
    var hash, text;
    text = "__VERSION__" + this.version + "__RESOURCE__" + this.resource + "__URL__" + this.fullURL + "__DESC__" + this.description;
