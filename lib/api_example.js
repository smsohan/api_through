(function() {
  var ApiExample, ApiHost, CUSTOM_HEADERS, VERSION_IN_HEADER, VERSION_IN_URL, mongoose, url, _u;
  ...
  ApiExample.schema.pre('save', function(callback) {
    ...
    ApiHost.update(upsertData, upsertData, {
      upsert: true
    }, function(error) {
      if (error != null) {
        return console.log("Failed to upsert host due to " + error);
      }
    });
    return callback();
  });
  ...
  ApiExample.prototype.populateFromRequest = function(request) {
    ...
    return this.resource = request.headers[CUSTOM_HEADERS.RESOURCE_HEADER] || this.guessedResource();
  };
